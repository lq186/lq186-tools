...
import com.lq186.tools.util.FileUtils;
import com.lq186.tools.util.StringUtils;

import java.io.File;

public final class CopyMavenJarExecImpl implements ICmdExec {
    ...
    private void doCopyFiles(String fromFolder, String fromFileName, String linePrefixes) throws Exception {
        ...
        FileUtils.doWithLine(fromFileName, line -> {
            if (lineMatch(line, linePrefixes)) {
                String jarDefine = line.substring(linePrefixes.trim().length()).trim();
                String[] jarDefines = jarDefine.split(":");
                String groupId = jarDefines[0];
                String artifactId = jarDefines[1];
                String type = jarDefines[2];
                String version = jarDefines[3];

                String jarName = StringUtils.join(artifactId, "-", version, ".", type);

                String groupIdPath = groupId.replace(".", File.separator);
                String srcFilePath = StringUtils.join(groupIdPath, File.separator, artifactId, File.separator, version, File.separator, jarName);
                File srcFile = new File(fromFolder, srcFilePath);
                if (srcFile.exists()) {
                    Print.printLine("复制文件：", srcFilePath);
                    File destFile = new File("mvn-lib", jarName);
